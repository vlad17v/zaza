...
private:
    void read_first_byte() {
        asio::async_read(socket_, asio::buffer(&first_byte_, 1),
    ...
    void read_stun_header() {
        asio::async_read(socket_, asio::buffer(stun_header_buf_),
            [self = shared_from_this()](boost::system::error_code ec, size_t) {
                if (ec) { self->owner_.removeSession(self->key_); return; }
    ...
    void read_stun_body(std::vector<uint8_t> header, uint16_t attrs_len) {
        ...
        asio::async_read(socket_,
            ...
            [self = shared_from_this(), body](boost::system::error_code ec, size_t) {
                if (ec) { self->owner_.removeSession(self->key_); return; }
                if (self->on_receive_)
    ...
    void read_channel_header() {
        asio::async_read(socket_, asio::buffer(chan_header_buf_),
            [self = shared_from_this()](boost::system::error_code ec, size_t) {
                if (ec) { self->owner_.removeSession(self->key_); return; }
                uint16_t data_len =
...
TcpTransport::TcpTransport(asio::io_context&  ioc,
...
void TcpTransport::onReceive(ReceiveCallback callback) {
...
void TcpTransport::send(const std::vector<uint8_t>& data,
    ...
    if (it != sessions_.end())
        ...
        std::cerr << "[tcp] no session for " << key << "\n";
}
        ...
        [this](boost::system::error_code ec, tcp::socket socket) {
            if (!ec) {
                auto session = std::make_shared<TcpSession>(
                    std::move(socket), on_receive_, *this);
                {
                    std::lock_guard<std::mutex> lock(sessions_mutex_);
...
void TcpTransport::removeSession(const std::string& key) {
    std::lock_guard<std::mutex> lock(sessions_mutex_);
    sessions_.erase(key);
}
